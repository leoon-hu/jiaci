...
| 设置 | 每日新词量、复习上限、学习顺序、新词顺序、口音、声音、例句小喇叭位置、自动发音、自动朗读、主题、词条资料来源、导出数据、注销，以及隐私政策与服务条款；「关于」里显示当前版本（构建时刻），「检查更新」和服务端正在跑的版本比对，有新版本就刷新到新版，还不行可以清除缓存重新加载；跑步模式的词数 / 遍数 / 释义 / 例句读法与条数 / 间隔 / 语速 / 播放时保持亮屏在跑步页里改，随账号保存 |
