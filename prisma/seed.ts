...
const CONFIG: Array<[string, string, string]> = [
  ...
  ["tts.concurrency", "6", "合成并发上限（批量脚本与按需合成共用）"],
  ...
  ["tts.synth_per_minute", "1000", "每个用户每分钟按需合成的段数上限（只防失控的客户端；跑步模式准备一轮要现合成几百段，别设太小）"],
  ["tts.daily_synth_per_user", "5000", "每个用户每天按需合成的段数上限"],
