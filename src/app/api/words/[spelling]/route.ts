 ...
 * `book` = 从哪本词库点进来的，状态与学习记录按那本的进度作用域算（需求 3.3.6）；不带按当前学习词库
 */
export const GET = withUser(async (req, ctx: Params<{ spelling: string }>, user) => {
  ...
  return ok(await buildWordDetail(user.id, word, resolveToday(u.searchParams.get("date")), u.searchParams.get("book") || null));
});
