...
export function middleware(req: NextRequest) {
  ...
  const origin = () => {
    ...
    return `${proto}://${allowed && claimed !== allowed ? allowed : claimed}`;
  };
  // 末尾带斜杠的地址：Next 默认在进中间件之前就把 /x/ 308 到 /x，于是不存在的路径也先「重定向」一次才 404，
  // 搜索引擎把这些都记成「网页会自动重定向」。改为自己处理（next.config 里 skipTrailingSlashRedirect）：
  // 确实有的页面照样 308 到不带斜杠的规范地址，没有的直接 404
  if (pathname.length > 1 && pathname.endsWith("/")) {
    const bare = pathname.replace(/\/+$/, "");
    if (classifyPath(bare) === "unknown") return pass();
    return NextResponse.redirect(new URL(bare + req.nextUrl.search, origin()), 308);
  }
  ...
  if (!req.cookies.get("aiword_session")?.value) {
    ...
    if (pathname.startsWith("/word/")) return NextResponse.redirect(`${origin()}/dict/${pathname.slice(6).replace(/%20| /g, "_")}`);
    return NextResponse.redirect(`${origin()}/login?next=${encodeURIComponent(pathname)}`);
  }
  return pass();
}
